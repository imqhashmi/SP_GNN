...
# Global best model tracking (similar to generatedIndividuals.py)
best_model_info = {
    'model_state': None,
    'loss': float('inf'),
    'accuracy': 0,
...
# Household Size Accuracy Function
def calculate_household_size_accuracy(assignments, household_sizes):
    """
    Calculate household size accuracy by comparing predicted vs expected household sizes.
    Returns both absolute accuracy and relative accuracy.
    """
    num_households = household_sizes.size(0)
    
    # Count how many people are assigned to each household
    predicted_counts = torch.zeros_like(household_sizes, device=household_sizes.device)
    for household_idx in assignments:
        predicted_counts[household_idx] += 1
    
    # Calculate household size accuracy (households with correct size / total households)
    correct_size_households = (predicted_counts == household_sizes).sum().item()
    size_accuracy = correct_size_households / num_households
    
    # Calculate relative size accuracy (1 - relative error)
    total_expected_size = household_sizes.sum().item()
    total_predicted_size = predicted_counts.sum().item()
    size_errors = torch.abs(predicted_counts - household_sizes).sum().item()
    relative_size_accuracy = 1.0 - (size_errors / (2 * total_expected_size)) if total_expected_size > 0 else 0.0
    
    return size_accuracy, relative_size_accuracy
...
def calculate_all_accuracies(assignments, person_nodes, household_nodes, household_sizes):
    ...
    # Calculate household size accuracy
    size_accuracy, relative_size_accuracy = calculate_household_size_accuracy(
        assignments, household_sizes
    )
    
    # Calculate overall average accuracy
    overall_accuracy = (religion_compliance + ethnicity_compliance + relative_size_accuracy) / 3.0
    
    return {
        'religion_compliance': religion_compliance,
        'ethnicity_compliance': ethnicity_compliance, 
        'size_accuracy': size_accuracy,
        'relative_size_accuracy': relative_size_accuracy,
...
def train_model(learning_rate, hidden_channels, return_detailed_results=False):
    ...
    for epoch in range(num_epochs):
        ...
        convergence_data['size_accuracies'].append(accuracies['size_accuracy'] * 100)
        convergence_data['relative_size_accuracies'].append(accuracies['relative_size_accuracy'] * 100)
...
performance_summary = {
    ...
    'best_size_accuracy': best_params['size_accuracy'],
...
best_params_with_loss['size_accuracy_percent'] = best_params['size_accuracy'] * 100
...
best_params_with_loss['final_size_accuracy'] = final_accuracies['relative_size_accuracy']
...
best_params_with_loss['final_size_accuracy_percent'] = final_accuracies['relative_size_accuracy'] * 100
...
print(f"  Size Accuracy: {final_accuracies['relative_size_accuracy'] * 100:.2f}%")
